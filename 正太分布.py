...
if __name__ == '__main__':
    ...
    for i in range(0,num):
        ...
        print("第%d个红包金额:%f" % ((i+1),round((min+result),2)))
        print("剩余金额:%f" % ((min * (num - i -1)) if (i != num -1 and total == 0) else (total + (min * (num -i -1)))))
    print nums,sum(nums)
    ...
    plt.title('Red envelopes: $\mu = %f$, $\sigma=%f$' % (round(average,2),round(sigma,2)))
